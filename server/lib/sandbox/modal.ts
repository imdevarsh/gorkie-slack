...
function getBaseImage() {
  return modal.images.fromRegistry(config.baseImage);
}

export async function createModalSandbox(imageId?: string): Promise<Sandbox> {
  const app = await getApp();
  const image = imageId ? await modal.images.fromId(imageId) : await getBaseImage();
...
export async function writeSandboxFiles(
  ...
  for (const file of files) {
    const mkdir = await sandbox.exec(
      ['sh', '-c', `mkdir -p "$(dirname "$1")"`, '--', file.path],
      { mode: 'text' }
    );
    await mkdir.wait();
