...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Find the uploaded 'photo.png' in attachments/ and invert it to black and white using ImageMagick. Save result, then call showFile to upload it to Slack." }</input></tool>
...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Generate a CSV report from the data. Save result, then call showFile to upload it to Slack." }</input></tool>
...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Download the video from https://www.youtube.com/watch?v=dQw4w9WgXcQ into output/ as MP4. Rename to a semantic filename, then call showFile to upload it. If needed, install yt-dlp and ffmpeg. If download requires auth or fails, include the exact error in the summary." }</input></tool>
