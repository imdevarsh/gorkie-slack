...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Find the uploaded 'photo.png' in attachments/ and invert it to black and white using ImageMagick. Save the result and return what you changed." }</input></tool>
...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Generate a CSV report from the data and save it under output/. Return the file path and a brief summary." }</input></tool>
...
<workflow>
<tool><name>sandbox</name><input>{ "task": "Download the video from https://www.youtube.com/watch?v=dQw4w9WgXcQ into output/ as MP4. Rename it semantically. If needed, install yt-dlp and ffmpeg. Return the final path and any errors." }</input></tool>
<tool><name>reply</name><input>{ "content": ["Done. I downloaded the video and saved it in output/."] }</input></tool>
</workflow>
</example>
