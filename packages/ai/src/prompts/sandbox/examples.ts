...
<steps>
1. glob({ "pattern": "**/*.png", "path": "attachments", "status": "Finding uploaded png" })
   → attachments/photo.png
2. bash({ "command": "sudo apt-get install -y imagemagick", "status": "Installing ImageMagick" })
3. bash({ "command": "mv attachments/photo.png attachments/cat-original.png && convert attachments/cat-original.png -colorspace Gray output/cat.png", "status": "Converting to grayscale" })
4. showFile({ "path": "output/cat.png", "title": "Black and white", "status": "Uploading result image" })
...
<steps>
1. bash({ "command": "convert output/cat.png -negate output/cat-inverted.png", "status": "Inverting image colors" })
2. showFile({ "path": "output/cat-inverted.png", "title": "Inverted", "status": "Uploading inverted image" })
...
2. bash({ "command": "mv attachments/diagram.png attachments/diagram-original.png && convert attachments/diagram-original.png -negate output/diagram.png", "status": "Negating diagram colors" })
3. showFile({ "path": "output/diagram.png", "title": "Inverted diagram", "status": "Uploading diagram output" })
