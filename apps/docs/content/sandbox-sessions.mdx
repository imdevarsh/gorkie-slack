...
1. Call `session.detach()`.
2. Strip `continueFrom` before storing normal resume state.
3. Find the Pi session file name.
4. Read session file bytes from the sandbox if available.
5. Store the new `resumeState` and optional session-file mirror in Postgres.
