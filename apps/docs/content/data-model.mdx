...
Do not add a parallel full Slack transcript just to make the model remember. The planned bounded Slack context preload should fetch recent messages each turn and tell Pi the limits. Pi's own history remains the durable conversation memory.
