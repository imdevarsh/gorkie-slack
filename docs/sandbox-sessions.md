---
title: Sandbox And Sessions
description: E2B, Pi host mirrors, resume state, session files, and skills.
---

The sandbox is a persistent Linux workspace for one Slack conversation. It is not where the Pi brain runs.

Pi runs on the bot machine. The sandbox gives Pi remote filesystem and shell access.

## Runtime Layout

```mermaid
flowchart TB
  subgraph Host["bot host machine"]
    Pi["Pi Node library"]
    HostWorkspace["tmp host workspace mirror"]
    HostAgent["tmp host agent dir\nSYSTEM.md, auth, model registry"]
    HostSessions["tmp host sessions"]
  end

  subgraph E2B["E2B sandbox"]
    Workdir["/home/user/pi-<thread>"]
    Skills["/home/user/.agents/skills"]
    SessionFiles["/home/user/pi-<thread>/.pi-sessions"]
  end

  Pi --> HostWorkspace
  Pi --> HostAgent
  Pi --> HostSessions
  HostWorkspace <-->|mirror| Workdir
  Pi -->|remote ops| Workdir
  Pi -->|read/write/bash| E2B
  Skills --> Pi
  SessionFiles --> HostSessions
```

The upstream Pi adapter confirms this: `createPi` is an in-process Node adapter with no bridge. It snapshots sandbox state into a host mirror, mounts the sandbox path through a VFS, and maps Pi's file operations back to sandbox operations.

## E2B Provider

`packages/sandbox/src/provider.ts` implements `HarnessV1SandboxProvider`.

It can:

- create a new E2B sandbox from `gorkie-sandbox:3.0`;
- resume a stored sandbox by id;
- detect missing/stale sandboxes and create a fresh one;
- pause a sandbox after a completed turn;
- update `sandbox_sessions` rows with active/paused runtime state.

`packages/sandbox/src/session.ts` adapts E2B to the AI SDK sandbox interface:

- `readTextFile`, `readBinaryFile`;
- `writeTextFile`, `writeBinaryFile`;
- `run`;
- `spawn`;
- `restricted()`;
- network infra methods from the full session.

## Session Persistence

There are two different things:

| Thing | Stored where | Meaning |
| --- | --- | --- |
| `resumeState` | Postgres text | Pointer that tells Harness/Pi which session file to reopen. |
| Pi session file | Sandbox plus Postgres mirror | Actual transcript and tool history. |

`packages/ai/src/sessions.ts` opens and persists sessions.

1. Load `sandbox_sessions.resumeState`.
2. Strip stale `continueFrom` from old unfinished turns.
3. Call `agent.createSession({ sessionId, resumeFrom })`.
4. On completion, call `session.detach()` and strip `continueFrom` before storing normal resume state.
5. Find the Pi session file, read it from the sandbox when available, and mirror it in Postgres.

```mermaid
sequenceDiagram
  participant Bot
  participant Harness
  participant Sandbox
  participant DB

  Bot->>DB: get resumeState + mirrored session file
  Bot->>Sandbox: create or resume E2B
  Bot->>Sandbox: seed mirrored .pi-sessions file if needed
  Bot->>Harness: createSession(resumeFrom)
  Harness->>Sandbox: run Pi tools against workspace
  Bot->>Harness: detach()
  Harness->>Sandbox: persist session file
  Bot->>Sandbox: read session file bytes
  Bot->>DB: update resumeState + mirror
  Bot->>Sandbox: pause
```

## Skills

Gorkie's runtime skills are **vendored on the host** under `packages/sandbox/skills/`:

- `agent-browser`
- `agentmail`

`loadSkills()` (`packages/sandbox/src/skills.ts`) reads those folders once and the bot passes them into Harness `skills`. The Pi adapter materializes Harness-provided skills into the sandbox itself — it writes each `SKILL.md` (and its files) under `$HOME/.agents/skills/<name>/` and registers them with the Pi resource loader.

Because Pi writes the skills into the sandbox, the host never reads them back out of E2B, and the template no longer installs `SKILL.md` files. The template still installs the **executables** the skills drive (the `agent-browser` CLI and the `agentmail` Python package).

## Template Build

`packages/sandbox/src/scripts/build-template.ts` builds the E2B template. It installs base tools, Node 24, Python packages, the `agent-browser` CLI, and browser dependencies. Skill `SKILL.md` files are not installed here — Pi materializes the host-vendored skills into the sandbox at session start.

Root command:

```sh
bun run build:template
```

The script loads `apps/bot/.env` through `dotenv` so `E2B_API_KEY` does not need to be tracked in code.
